    ...
    stages {
        ...
        stage('Soapui Test') {
            steps {
				...
				sh '$WORKSPACE/thirdparty/SoapUI-5.4.0/bin/testrunner.sh $WORKSPACE/project/soapui_projects/PocPipeline-soapui-project.xml -a -f$WORKSPACE/results/instance_$BUILD_NUMBER'
            }
        }
        stage('Reporting') {
            steps {
